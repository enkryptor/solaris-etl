...
- Класс `WriterSQL` — инициализируется из переменных окружения. Метод `start()` принимает отметку времени и создаёт новую сессию записи данных на это время. Метод `write()` позволяет записать данные об одном объекте. Метод `end()` заканчивает сессию записи и закрывает соединение.
