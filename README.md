...
Экспортируются классы отдельно для чтения и записи данных.

- Класс `ReaderCSV` — в конструкторе указываем путь к файлу. Далее метод `read()` возвращает записи из CSV построчно в виде асинхронного итератора.

- Класс `WriterSQL` — инициализируется из переменных окружения. Метод `start()` принимает отметку времени и создаёт новую сессию записи данных на это время. Метод `write()` позволяет отправить на запись данные об одном объекте (конкретный момент записи зависит от реализации). Метод `end()` заканчивает сессию, записывает оставшиеся данные и закрывает соединение.
...
Отдельная таблица замеров нужна, чтобы различать отсутствие данных и исчезновение объекта. Если замер существует, но состояния объекта в нём не нашлось — это явно говорит о том, что на момент замера объекта не было.

Работать с таблицей данных ФХЗ как со справочником, не дублировать для разных объектов — не оптимально, т.к. замедлит запись, при том что шансы совпадения ФХЗ для разных объектов околонулевые. Не делаем выборку по таблице блобов (по хэшам допустимо).
